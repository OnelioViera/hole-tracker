  ...
  useEffect(() => {
    const loadProject = async () => {
      try {
        ...
        // If no project parameter, check if we should load from localStorage
        // Only load from localStorage if we're not explicitly creating a new project
        const savedProjectId = localStorage.getItem('currentProjectId');
        const savedProject = localStorage.getItem('currentProject');
        
        // Check if user explicitly wants a new project (via URL hash or query param)
        const isNewProject = window.location.hash === '#new' || searchParams?.get('new') === 'true';
        
        if (isNewProject) {
          // User wants a new project, clear everything
          setProject({
            customerName: '',
            jobNumber: '',
            jobName: '',
        ...
        if (savedProjectId && savedProject && !isNewProject) {
          try {
        ...
        // Don't auto-load most recent project - start with empty form
        // This ensures "New Project" always shows a fresh form
        // If we get here without a project param, start fresh
        setProject({
          customerName: '',
          jobNumber: '',
          jobName: '',
